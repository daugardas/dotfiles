...
# set device source and global audio target variable
audio_target="" # Initialize audio_target
...
do
    ...
    i) nsink=$(pamixer --list-sources | grep "_input." | head -1 | awk -F '" "' '{print $NF}' | sed 's/"//')
        srce="--default-source"
        dvce="mic" 
        audio_target="@DEFAULT_AUDIO_SOURCE@" # This is for wpctl 
        ;;
    o) nsink=$(pamixer --get-default-sink | grep "_output." | awk -F '" "' '{print $NF}' | sed 's/"//')
        srce=""
        dvce="speaker" 
        audio_target="@DEFAULT_AUDIO_SINK@" # This is for wpctl ;;
    esac
done
...
case $1 in
    i) wpctl set-volume -l 1.4 "$audio_target" ${step}%+
        notify_vol ;;
    d) wpctl set-volume -l 1.4 "$audio_target" ${step}%-
        notify_vol ;;
    m) wpctl set-mute "$audio_target" toggle
        notify_mute ;;
    *) print_error ;;
esac
